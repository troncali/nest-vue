// From Vuepress Default Theme: https://github.com/vuepress/vuepress-next/blob/main/packages/%40vuepress/theme-default/src/client/components/global/CodeGroup.ts
	...
	setup(_, { slots }) {
		...
		return () => {
			...
			const items = (slots.default?.() || [])
				.filter(
					(vnode) =>
						(vnode.type as Component).name === "CodeGroupItem"
				)
				.map((vnode) => {
					if (vnode.props === null) {
						vnode.props = {};
					...
					return vnode as VNode & {
						props: Exclude<VNode["props"], null>;
					};
				});
			...
			if (activeIndex.value < 0 || activeIndex.value > items.length - 1) {
				...
				activeIndex.value = items.findIndex(
					(vnode) =>
						vnode.props.active === "" || vnode.props.active === true
				);
